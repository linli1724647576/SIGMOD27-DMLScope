...
\textbf{Tested DBMS.}
We evaluate \toolname\ on six widely used DBMSs, as summarized in Table~\ref{table:testdbms}. 
...
\begin{table}[h]
	...
	\resizebox{\columnwidth}{!}{
		\begin{tabular}{lcccc}
			...
			Percona~\cite{percona}      & 8.4.8-8       & 1.2K         & 122               & 2008          \\
...
Overall, \toolname\ detected 25 logical bugs, 8 abnormal errors, and 18 crashes, yielding 51 unique bugs in total. 
The logical bugs are distributed across all tested systems, including 5 in MySQL, 3 in MariaDB, 9 in MonetDB, 5 in Percona, 1 in DuckDB, and 2 in TiDB. 
Among all reported bugs, 51 have been confirmed by developers, and 31 have already been fixed. 
...
\begin{table}[t]
	\centering
	\caption{\toolname\ founded 51 unique bugs in six DBMSs.}
	\vspace{-2mm}
...
%\begin{table}[t]
%	\centering
%	\caption{\toolname\ found \textcolor{red}{47} unique bugs in six DBMSs.}
%	\vspace{-2mm}
%	\label{tab:bug_summary}
%	\resizebox{\columnwidth}{!}{
%		\begin{tabular}{lccccccc}
%			\toprule
%			\multirow{2}{*}{\textbf{DBMS}} & \multicolumn{3}{c}{\textbf{Bug type}} & \multicolumn{3}{c}{\textbf{Bug status}} \\
%			\cmidrule(lr){2-4} \cmidrule(lr){5-7}
%			& \textbf{Logical} & \textbf{Error} & \textbf{Crash} & \textbf{Confirmed} & \textbf{Fixed} & \textbf{Waiting} \\
%			\midrule
%			MySQL      & 5 & 0 & 3 & 8 & 0 & 0 \\
%			MariaDB    & 3 & 0 & 0 & 3 & 0 & 0 \\
%			MonetDB    & 9 & 8 & 12 & 26 & 26 & 3 \\
%			Percona    & 5 & 0 & 1 & 6 & 0 & 0 \\
%			DuckDB     & 1 & 0 & 2 & 3 & 2 & 0 \\
%			TiDB       & 2 & 0 & 0 & 2 & 0 & 0 \\
%			\midrule
%			Total      & 25 & 8 & 18 & 47 & 27 & 0 \\
%			\bottomrule
%		\end{tabular}
%	}
%	\vspace{-3mm}
...
Specifically, \toolname\ generates 46.0 test cases/sec for MySQL, 54.8 for MariaDB, 48.4 for MonetDB, 51.6 for Percona, 44.7 for DuckDB, and 49.3 for TiDB. 
Beyond competitive throughput, \toolname\ also attains high execution success rates on most DBMSs, reaching 93.8\% for MySQL, 93.6\% for MariaDB, 93.7\% for MonetDB, 92.3\% for Percona, 89.3\% for DuckDB, and 88.5\% for TiDB.
We further measure the average complexity of generated statements using their size in bytes. 
The average statement size is 1,684 bytes for MySQL, 1,330 for MariaDB, 1,414 for MonetDB, 1,547 for Percona, 1,625 for DuckDB, and 1,482 for TiDB. 
...
\textbf{Bug Diversity.}
We analyze the \logical\ confirmed logical bugs from two perspectives: the diversity of DML types and the structural complexity of the triggering statements. 
In terms of DML types, the majority of bugs are caused by \texttt{UPDATE} statements (15/25), while the remaining involve \texttt{DELETE} (7/25) and \texttt{INSERT} (3/25), indicating that value-modifying operations are particularly error-prone. 
From a structural perspective, most bugs are triggered by complex query constructs: 22 out of 25 bugs involve subqueries, 9 involve joins, 12 involve set operations, and 16 involve logic operators. 
...
\begin{table}[t]
	...
	\resizebox{\columnwidth}{!}{
		\begin{tabular}{ccccccc}
			...
			13 & \texttt{MonetDB\#7897} & DELETE & \cmark & \xmark & \cmark & \xmark \\
			14 & \texttt{MonetDB\#7912} & INSERT & \cmark & \xmark & \xmark & \cmark \\
			15 & \texttt{MonetDB\#7913} & DELETE & \cmark & \xmark & \cmark & \xmark \\
			16 & \texttt{MonetDB\#7914} & DELETE & \cmark & \xmark & \cmark & \xmark \\
			17 & \texttt{MonetDB\#7915} & DELETE & \cmark & \xmark & \cmark & \xmark \\
			
			18 & \texttt{Percona\#10877} & UPDATE & \cmark & \cmark & \xmark & \cmark \\
			19 & \texttt{Percona\#10878} & UPDATE & \cmark & \cmark & \xmark & \cmark \\
			20 & \texttt{Percona\#10879} & UPDATE & \cmark & \cmark & \cmark & \cmark \\
			21 & \texttt{Percona\#10906} & UPDATE & \xmark & \cmark & \xmark & \cmark \\
			22 & \texttt{Percona\#11008} & UPDATE & \cmark & \xmark & \xmark & \xmark \\
			
			23 & \texttt{DuckDB\#21609} & INSERT & \cmark & \cmark & \xmark & \cmark \\
			
			24 & \texttt{TiDB\#67018} & UPDATE & \cmark & \xmark & \cmark & \xmark \\
			25 & \texttt{TiDB\#67019} & DELETE & \cmark & \xmark & \cmark & \cmark \\
...
\textbf{Case Study and False Positive.}
Due to space limitations, we present additional representative and interesting bug cases at \url{https://anonymous.4open.science/r/DMLScope-9F03/CaseStudy.md}.  
In an early prototype, we observed several false positives caused by unstable DML execution semantics, which were eliminated after incorporating stability-aware state-effect extraction. 
...
To illustrate the effectiveness of our approach, we conduct our comparative study from two perspectives: tool level and oracle level. 
...
%
At the oracle level, we manually analyze whether existing testing oracles are capable of detecting the logical bugs uncovered by \toolname. 
The results show that the \logical\ bugs detected by \toolname\ are entirely disjoint from the 12 logical bugs triggered by the baseline tools.
...
This highlights that \toolname\ explores a different and previously untested space of DML correctness.
